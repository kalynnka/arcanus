...
class BaseTransmuter(BaseModel, metaclass=TransmuterMetaclass):
    ...
    @classmethod
    def pre_formulate(cls, data: Any, info: ValidationInfo) -> Any:
        ...
        if provider is not None and isinstance(data, provider):
            ...
            if instance is None or instance.__transmuter_revalidating__:
                ...
                loaded.raw_data = data  # type: ignore
                return loaded
            if instance:
                return instance
    ...
    def post_formulate(self, info: ValidationInfo) -> BaseTransmuter:
        # Get materia once to avoid repeated ContextVar lookups
        materia = active_materia.get()

        # Handle NoOpMateria case - fast path using identity check
        if materia is _noop_materia:
            object.__setattr__(self, "__transmuter_provided__", None)
            object.__setattr__(self, "__transmuter_revalidating__", False)
            return self
        ...
        if (
            ...
            and self.raw_data
            and isinstance(self.raw_data, provider)
        ):
            context = validated.get()
            cached = context.get(self.raw_data)

            if self.raw_data.transmuter_proxy is None:
                object.__setattr__(self, "__transmuter_provided__", self.raw_data)
                object.__setattr__(self, "__transmuter_revalidating__", False)

                self.raw_data.transmuter_proxy = self
                instance = materia.transmuter_after_validator(self, info)

                if not cached:
                    context[self.raw_data] = instance
            ...
            if provider is not None:
                ...
                object.__setattr__(self, "__transmuter_provided__", provided)
                object.__setattr__(self, "__transmuter_revalidating__", False)
            else:
                object.__setattr__(self, "__transmuter_provided__", None)
                object.__setattr__(self, "__transmuter_revalidating__", False)

        for name in cls.model_associations.keys() & self.model_fields_set:
